...
//Datos de WiFi
const char* ssid = "Totalplay-319F_2.4Gnormal";  // Aquí debes poner el nomrrrrrrrrrbre de tu red
const char* password = "319FEF5D88K2kS97";  // Aquí debes poner la contraseña de tu red
...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("esp32/relay/entrada", dataString); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32/relay/foco") {  // En caso de recibirse mensaje en el tema esp32/output
    if(messageTemp == "true"){
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    // Intentar reconexión
    if (client.connect("ESP32Client_1")) { //Pregunta por el resultado del intento de conexión
      Serial.println("Conectado");
      client.subscribe("esp32/relay/foco"); // Esta función realiza la suscripción al tema
